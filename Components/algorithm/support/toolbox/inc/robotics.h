...
public:
    ...
    Matrixf<_n, 1> ikine(
        ...
        for (int i = 0; i < max_iter; i++) {
            ...
            for (int j = 0; j < 5; j++) {
                ...
                {
                    dq = matrixf::inv(J.trans() * J + 0.1f * matrixf::eye<_n, _n>()) * J.trans()
                        * err * step;
    ...
    rne(Matrixf<_n, 1> q,
        ...
        for (int i = 1; i <= _n; i++) {
            ...
            b_i = b.col(i - 1) + vector3f::cross(w.col(i - 1), qv[i - 1][0] * R_0iminus1 * ez)
        ...
        for (int i = _n; i > 0; i--) {
            ...
            f_iminus1 = f.col(i) + links_[i - 1].m() * ac.col(i) - links_[i - 1].m() * gravity_;
            // μ_i-1 = μ_i+f_ixrc_i-f_i-1xrc_i-1->ci+I_i*b_i+ω_ix(I_i*ω_i)
            mu_iminus1 = mu.col(i) + vector3f::cross(f.col(i), R_0i * links_[i - 1].rc())
                - vector3f::cross(f_iminus1, R_0i * (RT_iminus1i * P_iminus1i + links_[i - 1].rc()))
